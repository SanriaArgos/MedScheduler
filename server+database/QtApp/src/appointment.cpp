...
void Appointment::handle_day_clicked(QPushButton* clicked_button,int days) {
    ...
    nlohmann::json schedule_json = client.get_doctor_schedule_for_patient(doctor_id);
    auto schedule_array = schedule_json["schedule"];
    
     for (const auto& slot_json : schedule_array){
        slot sl;
        sl.appointment_date = QDate::fromString(
    QString::fromStdString(slot_json["appointment_date"].get<std::string>()),
    "yyyy-MM-dd"
);
        if(sl.appointment_date!=date) continue;
        sl.appointment_time = QTime::fromString(
        QString::fromStdString(slot_json["appointment_time"]), 
        "hh:mm:ss");
        sl.full_name = QString::fromStdString(slot_json["full_name"]);
        sl.house = QString::fromStdString(slot_json["house"]);
        sl.junior_admin_phone = QString::fromStdString(slot_json["junior_admin_phone"]);
        sl.region = QString::fromStdString(slot_json["region"]);
        sl.settlement_name = QString::fromStdString(slot_json["settlement_name"]);
    ...
    for (const slot &s : slot_vector) {
        ...
        slotLayout->addWidget(new QLabel(QString("Hospital phone: %1").arg(s.junior_admin_phone)));
...
void Appointment::on_date0_clicked() { handle_day_clicked(ui->date0,0); }
void Appointment::on_date1_clicked() { handle_day_clicked(ui->date1,1); }
void Appointment::on_date2_clicked() { handle_day_clicked(ui->date2,2); }
void Appointment::on_date3_clicked() { handle_day_clicked(ui->date3,3); }
void Appointment::on_date4_clicked() { handle_day_clicked(ui->date4,4); }
void Appointment::on_date5_clicked() { handle_day_clicked(ui->date5,5); }
void Appointment::on_date6_clicked() { handle_day_clicked(ui->date6,6); }
