...
std::string sendSmsWithAuth(
    ...
    if (curl) {
        ...
        curl_easy_setopt(
            curl, CURLOPT_POSTFIELDSIZE, (long)xmlPayload.length()
        );
        ...
        // Отключаем проверку SSL сертификата (как в PHP примере, но не
...
sendSms(const std::string &phoneNumber, const std::string &textMessage) {
    return sendSmsWithAuth(
        SMS_API_LOGIN, SMS_API_PASSWORD, phoneNumber, textMessage
    );
}