...
void edit_doctor_feedback(
    ...
    if (!data.contains("rating_id")   ||
        ...
        PGresult *chk = PQexecParams(
            ...
            "SELECT user_id FROM rating WHERE id=$1::int",
            1,nullptr,chk_p,nullptr,nullptr,0
        );
        if (!chk || PQntuples(chk)==0) {
            ...
            if (author != uid) {
                ...
                // 3) Выполняем UPDATE
                const char *upd_p[6] = {
                    text.c_str(), name.c_str(), date.c_str(),
                    std::to_string(rate).c_str(),
                    address.c_str(),
                    std::to_string(rid).c_str()
                };
                PGresult *upd = PQexecParams(
                    db_handler.get_connection(),
                    "UPDATE rating SET text=$1,name=$2,date=$3,rate=$4,address=$5 "
                    "WHERE id=$6::int",
