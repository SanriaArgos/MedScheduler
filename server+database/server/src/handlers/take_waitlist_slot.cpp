...
void take_waitlist_slot(
    ...
    if (!data.contains("doctor_id")
        || !data.contains("old_patient_id")
        || !data.contains("new_patient_id")
        || !data.contains("appointment_date")
        || !data.contains("appointment_time")
        || !data.contains("hospital_id")
    ) {
    ...
    // 3) Обновляем запись: заменяем старого пациента на нового и сбрасываем флаг cancelled
    {
        ...
        PGresult *res_update = PQexecParams(
            ...
            "WHERE doctor_id = $3 AND appointment_date = $4 AND appointment_time = $5 "
    ...
    // 4) Удаляем из таблицы waitlist запись только что обслуженного пациента
    {
        ...
        PGresult *res_del = PQexecParams(
            db_handler.get_connection(),
            "DELETE FROM waitlist WHERE doctor_id = $1 AND patient_id = $2",
            2, nullptr, params_del, nullptr, nullptr, 0
        );
        if (res_del) PQclear(res_del);
    }

    // 5) Вызываем SQL-функцию notify_next_waitlister, чтобы спустить уведомление следующему в очереди
    {
        const char *param_doc = std::to_string(doctor_id).c_str();
        PGresult *res_notify = PQexecParams(
            db_handler.get_connection(),
            "SELECT notify_next_waitlister($1)",
            1, nullptr,
            &param_doc,
            nullptr, nullptr, 0
        );
