...
get_junior_admin_hospital_id(int junior_admin_id, int &out_hospital_id) {
    ...
    PQclear(res);
    std::cerr
        << "Error: Could not retrieve hospital for junior administrator\n";
    return false;
}

void add_hospital_to_doctor(
    const json &data,
    ...
    if (!data.contains("doctor_id") || !data.contains("hospital_id") ||
        ...
        std::cerr
            << "Error: Missing required fields for add_hospital_to_doctor\n";
    ...
    // Проверяем, что заданная больница принадлежит данному младшему
    // администратору.
    ...
    PGresult *res_update = PQexecParams(
        db_handler.get_connection(),
        "UPDATE doctors SET hospital_ids = array_append(hospital_ids, $2) "
        "WHERE doctor_id = $1",
        2, NULL, params_update, NULL, NULL, 0
    );
    if (PQresultStatus(res_update) != PGRES_COMMAND_OK) {
        std::cerr << "Error adding Hospital ID: "
