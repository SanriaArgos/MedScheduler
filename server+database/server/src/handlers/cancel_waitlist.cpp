...
void cancel_waitlist(
    ...
    if (!data.contains("waitlist_id") || !data.contains("patient_id")) {
        ...
        const char *check_p[2] = {
            std::to_string(wl_id).c_str(),
            std::to_string(pid).c_str()
        };
        PGresult *chk = PQexecParams(
            db_handler.get_connection(),
            "SELECT 1 FROM waitlist WHERE id = $1::int AND patient_id = $2::int",
            2, nullptr, check_p, nullptr, nullptr, 0
        );
        bool ok = (chk && PQresultStatus(chk) == PGRES_TUPLES_OK && PQntuples(chk) > 0);
        PQclear(chk);
        ...
        if (!ok) {
            ...
            PGresult *del = PQexecParams(
                db_handler.get_connection(),
                "DELETE FROM waitlist WHERE id = $1::int",
                1, nullptr, check_p, nullptr, nullptr, 0
            );
