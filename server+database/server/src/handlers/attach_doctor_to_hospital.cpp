...
void attach_doctor_to_hospital(const json& body, http::response<http::string_body>& res, database_handler& db_handler) {
    ...
    // if (hospital_ids_str.find(std::to_string(hospital_id)) != std::string::npos) {
    //     result = {
    //         {"success", false},
    ...
    const char* update_params[2] = {std::to_string(doctor_id).c_str(), std::to_string(hospital_id).c_str()};
    PGresult* update_res = PQexecParams(conn,
    "UPDATE doctors SET hospital_ids = array_append(COALESCE(hospital_ids, '{}'::integer[]), $2::integer) WHERE doctor_id = $1", 2, nullptr, update_params, nullptr, nullptr, 0);
