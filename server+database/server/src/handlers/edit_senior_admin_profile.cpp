...
void edit_senior_admin_profile(
    ...
    if (data.contains("new_password")) {
        ...
        add_text_param("hashed_password", bytes_to_hex_senior(new_hash, SHA256_DIGEST_LENGTH));
        add_text_param("salt", bytes_to_hex_senior(reinterpret_cast<const unsigned char*>(new_salt.data()), new_salt.size()));
    }
