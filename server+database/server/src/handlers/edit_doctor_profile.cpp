...
void edit_doctor_profile(
    ...
    if (data.contains("new_password") || data.contains("new_password_repeat")) {
        ...
        SHA256(reinterpret_cast<const unsigned char*>(tohash.c_str()), tohash.size(), new_hash_bytes);
    ...
    if (!update_result || PQresultStatus(update_result) != PGRES_COMMAND_OK) {
        std::string error_msg = update_result ? PQerrorMessage(db_handler.get_connection()) : "Unknown error";
