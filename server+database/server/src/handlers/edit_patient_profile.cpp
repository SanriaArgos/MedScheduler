...
// Вспомогательная функция: проверяем, что (phone) уникален
...
void edit_patient_profile(
    ...
    // 2) Сначала проверим, что user_type = "patient"
    {
    ...
    // 3) Проверяем корректность current_password:
    //    достаём из БД current_hashed, salt, формируем новый хэш и сравниваем.
    ...
    // Считаем SHA256(current_password + db_salt):
    ...
    // 4) Собираем новый набор полей для UPDATE
    std::vector<std::string> clauses;
    std::vector<const char*>  params;
    int idx = 1;

    // Поля: last_name, first_name, patronymic, phone
    ...
    // 5) Если есть смена пароля, проверяем new_password и new_password_repeat
    ...
    if (data.contains("new_password") || data.contains("new_password_repeat")) {
        ...
        // Генерируем новую соль и новый хэш
