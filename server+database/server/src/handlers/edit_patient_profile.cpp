...
void edit_patient_profile(
    ...
    PGresult *type_result = PQexecParams(
        ...
        new const char *[] { std::to_string(user_id).c_str() }, nullptr,
        nullptr, 0
    );

    if (PQresultStatus(type_result) != PGRES_TUPLES_OK ||
        PQntuples(type_result) == 0) {
        PQclear(type_result);
        response["success"] = false;
    ...
    PGresult *creds_result = PQexecParams(
        ...
        nullptr, new const char *[] { std::to_string(user_id).c_str() },
        nullptr, nullptr, 0
    );
    ...
    auto add_hex_param = [&](const std::string &field, const std::string &hex_value) {
    ...
    if (data.contains("new_password")) {
        std::string new_pass = data["new_password"].get<std::string>();
        std::string repeat_pass = data["new_password_repeat"].get<std::string>();
        ...
        SHA256(
            reinterpret_cast<const unsigned char *>(to_hash.c_str()),
            to_hash.size(), new_hash
        );

        add_text_param("hashed_password", bytes_to_hex(new_hash, SHA256_DIGEST_LENGTH));
        add_hex_param("salt", bytes_to_hex(reinterpret_cast<const unsigned char *>(new_salt.data()), new_salt.size()));
    }
    ...
    PGresult *update_result = PQexecParams(
        conn, sql.c_str(), params.size(), nullptr, params.data(), nullptr,
        nullptr, 0
    );
