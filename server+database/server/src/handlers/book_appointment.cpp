...
void book_appointment(
    ...
    for (auto &f : {"doctor_id","hospital_id","appointment_date","appointment_time","patient_id"}) {
        if (!data.contains(f)) {
    ...
    std::string hospital_id_str  = std::to_string(data["hospital_id"].get<int>());
    ...
    const char *p1[4] = { doctor_id_str.c_str(), hospital_id_str.c_str(), date_str.c_str(), time_str.c_str() };
    ...
    char const *cur_pid = PQgetvalue(r1,0,0);
    if (cur_pid[0] != '\0') {
        PQclear(r1);
        response["success"] = false;
    ...
    const char *p2[5] = { doctor_id_str.c_str(), hospital_id_str.c_str(), date_str.c_str(), time_str.c_str(), patient_id_str.c_str() };
