...
static bool check_user_exists(PGconn *conn, const std::string &username) {
    ...
    PGresult *res = PQexecParams(
        conn, "SELECT 1 FROM pg_catalog.pg_user WHERE usename = $1", 1, NULL,
...
static bool check_database_exists(PGconn *conn, const std::string &dbname) {
    ...
    PGresult *res = PQexecParams(
        conn, "SELECT 1 FROM pg_database WHERE datname = $1", 1, NULL,
        paramValues, NULL, NULL, 0
    );
    bool exists =
        (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0);
...
bool initialize_system(const std::string &superuser_connect_info) {
    ...
    if (!check_user_exists(conn, "meduser")) {
        if (!execute_sql(
                conn, "CREATE USER meduser WITH PASSWORD '3671920119' CREATEDB"
            )) {
    ...
    const std::string meduser_conn_info =
        "dbname=postgres user=postgres password=123 host=db port=5432";
    PGconn *conn_meduser = PQconnectdb(meduser_conn_info.c_str());
    if (PQstatus(conn_meduser) != CONNECTION_OK) {
        std::cerr << "Meduser connection error: "
                  << PQerrorMessage(conn_meduser) << "\n";
    ...
    const std::string medscheduler_conn_info =
        "dbname=medscheduler user=postgres password=123 host=db port=5432";
    PGconn *conn2 = PQconnectdb(medscheduler_conn_info.c_str());
    if (PQstatus(conn2) != CONNECTION_OK) {
        std::cerr << "Medscheduler connection error (meduser): "
    ...
    if (PQstatus(conn3) != CONNECTION_OK) {
        std::cerr << "Medscheduler connection error (meduser): "
                  << PQerrorMessage(conn3) << "\n";
        PQfinish(conn3);
        return false;
    }
    const char *paramValues[1] = {"senior administrator"};
    PGresult *res = PQexecParams(
        conn3, "SELECT 1 FROM users WHERE user_type = $1", 1, NULL, paramValues,
        NULL, NULL, 0
    );
    bool senior_exists =
        (PQresultStatus(res) == PGRES_TUPLES_OK && PQntuples(res) > 0);
    ...
    if (!senior_exists) {
        ...
        PGresult *res_ins = PQexecParams(
            ...
            "user_type, hashed_password, salt) "
