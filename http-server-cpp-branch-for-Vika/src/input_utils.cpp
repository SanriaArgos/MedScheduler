...
std::string get_Validated_Phone() {
    while (true) {
        std::string phone = get_Validated_Input(
            "Телефон (с +, затем ровно 10 цифр, первая 7 или 8):", true
        );
        if (phone == CANCEL_COMMAND) {
            return phone;
        }
        if (!Auth::validate_Phone(phone)) {
            std::cout << "Ошибка: неверный формат телефона! Он должен "
                         "начинаться с '+' и содержать ровно 10 цифр, первая "
                         "из которых 7 или 8.\n";
            continue;
        }
        return phone;
...
std::string get_Validated_Password() {
    while (true) {
        std::string password = get_Validated_Input(
            "Пароль (не менее 8 символов, только латинские буквы, цифры и "
            "символы):",
            true
        );
        ...
        if (password.size() < 8) {
            std::cout
                << "Ошибка: пароль должен содержать не менее 8 символов!\n";
            continue;
        }
        if (!validate_Password_Characters(password)) {
            std::cout << "Ошибка: пароль может содержать только латинские "
