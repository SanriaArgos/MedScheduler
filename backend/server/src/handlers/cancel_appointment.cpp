...
void cancel_appointment(
    ...
    try {
        ...
        PGresult *wl = PQexec(
            db_handler.get_connection(), "SELECT COUNT(*) FROM waitlist"
        );
        ...
        if (queue_empty) {
            sql = "UPDATE records SET patient_id = NULL WHERE record_id = " +
                  ...
                  " AND patient_id = " + std::to_string(patient_id);
        } else {
            sql =
        ...
        if (!r || PQresultStatus(r) != PGRES_COMMAND_OK) {
            response["success"] = false;
            response["error"] =
                r ? PQresultErrorMessage(r) : "Null result from database";
            res.result(http::status::internal_server_error);
            if (r) {
                PQclear(r);
            }
            ...
            if (affected_rows > 0) {
                ...
                response["message"] = queue_empty
                                          ...
                                          : "Cancelled and notified waitlister";
                response["affected_rows"] = affected_rows;
                res.result(http::status::ok);
            } else {
                response["success"] = false;
                response["error"] =
                    "No matching record found or already updated";
